...
| 전공 | 임베디드 소프트웨어 |
| 핵심 기술 | MediaPipe, OpenCV, Ollama(로컬 LLM), edge-tts, Django, Docker, WSL2 |
...
### 4.3 회복 효과 검증 (RecoverySession)

```
흐름: recovering → evaluating → done

1. recovering: 가이드 수행 시간 대기 (얼굴 이탈 허용)
2. evaluating: 30초간 생체 데이터 재측정 (최소 10샘플)
3. 판정 (OR):
   - 피로도 10점 이상 하락
   - 평균 졸음 점수 ≤ 30점
   → 성공: 피로도 -30점 + 작업 타이머 리셋
   → 실패: 추가 휴식 권고
4. 결과를 recovery_actions 테이블에 저장
```

### 4.4 개인화 회복 프로필 (RecoveryProfile)

DB 회복 이력에서 가이드별 성공률을 계산하여 개인에게 효과적인 가이드 우선 추천.
- 성공률 30% 미만 → 추천 제외
- 성공률 70% 이상 → 보너스 추가 추천
- 최소 3회 시도 후 개인화 적용
...
│   ├── drowsiness.py        # EAR / MAR / PERCLOS 계산
