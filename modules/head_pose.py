...
class HeadPoseEstimator:
    ...
    def get_head_score(self, pitch, yaw, roll):
        ...
        - |roll| 큼: 고개 기울임 → 중간 점수
        """
        # Pitch 점수 (고개 숙임 — 졸음의 핵심 지표)
        # 데드존 15°: 화면을 내려다보는 정도는 무시
        pitch_score = _lerp_score(-pitch, [
            (0, 0), (15, 0), (20, 10), (28, 30),
            (38, 60), (50, 80), (65, 100),
        ])

        # Yaw 점수 제거 — 옆을 바라보는 동작은 졸음 징후가 아님
        yaw_score = 0

        # Roll 점수 (졸면서 머리가 옆으로 기울어지는 징후)
        # 데드존 12°: 자연스러운 기울임은 무시
        roll_score = _lerp_score(abs(roll), [
            (0, 0), (12, 0), (22, 10), (32, 25), (48, 40),
        ])
