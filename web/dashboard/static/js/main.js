...
async function updateDailyReport() {
    try {
        const res = await fetch(`${API_BASE}/daily_report/`);
...
async function updateLogs() {
    try {
        const res = await fetch(`${API_BASE}/logs/?page=1&limit=20`);
        const json = await res.json();
...
async function updateCharts() {
    try {
        const fatRes = await fetch(`${API_BASE}/fatigue/?period=today`);
        ...
        const envRes = await fetch(`${API_BASE}/environment/?hours=24`);
        ...
        const logRes = await fetch(`${API_BASE}/logs/?page=1&limit=100`);
